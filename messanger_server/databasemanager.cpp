...
{
    QSqlQuery query;
    query.prepare("SELECT 1 FROM chat_participants WHERE user_id = :user_id AND chat_id = :chat_id");
...
QList<int> DatabaseManager::getChatParticipants(int chatId)
{
    ...
    query.bindValue(":chatId", chatId);
    if(!query.exec()) return participants;
    while(query.next()) participants.append(query.value("user_id").toInt());
    return participants;
}
...
DatabaseManager::DbResult DatabaseManager::registerUser(const QString& username, const QString& password, int& outUserId)
{
    ...
    query.prepare("SELECT user_id FROM users WHERE username = :username");
    query.bindValue(":username", username);
    if(!query.exec()) return DB_ERROR;
    if(query.next()) return DB_USER_EXISTS;
...
DatabaseManager::DbResult DatabaseManager::getUserInfo(int userId, QVariantMap& outUser)
{
    ...
    if(!query.exec()) return DB_ERROR;
    if(!query.next()) return DB_USER_NOT_FOUND;
    ...
    QList<int> chatIds;
    if(chatQuery.exec()) {
        while(chatQuery.next()) chatIds.append(chatQuery.value("chat_id").toInt());
    }
...
{
    QSqlQuery query;
    query.prepare("INSERT INTO chats (chat_name, type, created_at, creator_id) "
...
DatabaseManager::DbResult DatabaseManager::getChatInfo(int chatId, QVariantMap& outChat)
{
    outChat.clear();
    QSqlQuery query;
    query.prepare("SELECT chat_name, type FROM chats WHERE chat_id = :chatId");
    query.bindValue(":chatId", chatId);
    if(!query.exec()) return DB_ERROR;
    if(!query.next()) return DB_ERROR;
    outChat["chat_name"] = query.value(0).toString();
    outChat["type"] = query.value(1).toString();
    return DB_OK;
}

DatabaseManager::DbResult DatabaseManager::getChatParticipantsUsernames(int chatId, QList<QVariantMap>& outUsers)
{
    outUsers.clear();
    QSqlQuery query;
    query.prepare(
...
DatabaseManager::DbResult DatabaseManager::addUserToChat(int chatId, int userId, const QString& role)
{
    ...
    if(!checkQuery.exec()) return DB_ERROR;
    if(checkQuery.next()) return DB_USER_EXISTS;
    QSqlQuery insertQuery;
    insertQuery.prepare("INSERT INTO chat_participants (chat_id, user_id, role) VALUES (:chat_id, :user_id, :role)");
    insertQuery.bindValue(":chat_id", chatId);
    insertQuery.bindValue(":user_id", userId);
    insertQuery.bindValue(":role", role);
    if(!insertQuery.exec()) return DB_ERROR;
    return DB_OK;
}
...
{
    QSqlQuery query;
    query.prepare("DELETE FROM chat_participants WHERE chat_id = :chat_id AND user_id = :user_id");
    query.bindValue(":chat_id", chatId);
    query.bindValue(":user_id", userId);
    if(!query.exec()) return DB_ERROR;
    return DB_OK;
...
DatabaseManager::DbResult DatabaseManager::findUsers(const QString& search, QList<QString>& outUsers)
{
    outUsers.clear();
    QSqlQuery query;
    query.prepare("SELECT username FROM users WHERE username LIKE :search AND role != 'admin'");
    query.bindValue(":search", "%" + search + "%");
    if(!query.exec()) return DB_ERROR;
    while(query.next()) outUsers.append(query.value(0).toString());
    if(outUsers.isEmpty()) return DB_USER_NOT_FOUND;
    return DB_OK;
}
